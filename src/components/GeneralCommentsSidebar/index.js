...
class GeneralCommentsSidebar extends Component {
  ...
  handleClickSendComment = () => {
    if (this.state.textToSubmit !== '')
      API.sendGeneralComment(this.context.authUser.token,this.props.discussionId, {text: this.state.textToSubmit})
      .then(comment => {this.setState({comments: [...this.state.comments, comment] });})
  }

  componentDidUpdate(prevProps) {
    if (this.props.visible && prevProps.visible !== this.props.visible)
      API.getGeneralComments(this.props.discussionId)
...
GeneralCommentsSidebar.contextType = AuthContext

export default GeneralCommentsSidebar;
